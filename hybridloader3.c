...
static void handler(int sig, siginfo_t* si, void* unused)
{
	...
	size_t aligned_vaddr = (uint64_t) si->si_addr - size_offset - sysconf(_SC_PAGE_SIZE);
	char* addr = (char*) mmap((void*) aligned_vaddr, sysconf(_SC_PAGE_SIZE) * 3, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
	if(addr == MAP_FAILED)
	{
